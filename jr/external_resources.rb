...
#saves it to a file
  File.open("Unique_Mesh_tags_from_resources.csv", "w") do |file|
...
puts new_mesh_array_terms
 ...
 File.open("new_mesh_array_terms.csv", "w") do |file|
...
puts "hi user if you would like to quit the program and add some of the above MeSH tags to the trekk site type 'yes'. If instead you would like to continue to add the external_resources without the new mesh array terms type 'no'"
puts '> '
...
File.open("external_resources_output.csv", "w") do |file|
        external_resources.each do |resource|
            file.write(resource)
        end 
    end
